...
4. Review `packages/pacman.txt` and `packages/aur.txt` by hand, then install what you actually
   want:
   ```
   sudo pacman -S --needed - <packages/pacman.txt
   yay -S --needed - <packages/aur.txt
   ```
   These are reference manifests, not run automatically by `install.sh` — `pacman -Qqe` already
   excludes packages pulled in purely as dependencies, but its explicit/dependency bookkeeping can
   drift over time, so treat the list as a starting point to confirm, not a blind restore.
5. Run `mise install` to fetch the language/tool versions from `config/mise/config.toml`.
6. Set up `~/.config/hypr/monitors.lua` by hand for the new machine's actual displays (see below).
...
- `packages/{pacman.txt,aur.txt,export-packages.sh}` — software installed beyond Omarchy's stock
  set. Regenerate with `packages/export-packages.sh`. Reference manifests, reviewed by hand before
  installing — not run automatically.

## What's deliberately not tracked

- `~/.config/hypr/monitors.lua` — real hardware profile, specific to whichever machine set it up.
  Each machine keeps its own, set up by hand.
...
## Open follow-up: replacing Ente Auth

`ente-auth-bin` (2FA/TOTP client) is AUR-only and regularly drifts out of date. Looking for
something with official packaging and built-in cloud sync:

- **Bitwarden Authenticator** — standalone TOTP app, sync via a Bitwarden account. First
  candidate to trial; still need to confirm current Arch packaging (official repo vs. AUR) and
  whether sync requires Premium.
- **KeePassXC** — official Arch `extra` package, TOTP built into entries. Fallback if Bitwarden's
  sync turns out to be paywalled or still AUR-only; its sync is bring-your-own (Syncthing, a
  cloud-mounted folder) rather than integrated.
